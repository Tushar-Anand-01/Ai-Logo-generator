...
const Hero = () => {
  ...
  return (
    <div className="flex items-center flex-col mt-24 gap-5">
      <h2 className="text-blue-600 text-5xl text-center font-bold">
        {LookUp.HeroHeading}
      </h2>
      ...
      <div className="flex gap-6 w-full  mt-10 items-center  justify-center">
        {user ? (
          <Link href="/create">
            <Button variant="outline" className="mt-5 bg-teal-600 hover:bg-teal-700 text-white">
              Create Your Logo
            </Button>
          </Link>
        ) : (
          <SignInButton mode="modal" forceRedirectUrl={"/create"}>
            <Button className="bg-teal-600 hover:bg-teal-700 text-white">Login to Generate</Button>
          </SignInButton>
        )}
      </div>
    </div>
